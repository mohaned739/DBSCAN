...
def MyDBSCAN(D, eps, MinPts):
    ...
    for point in range(size):
        if my_labels[point]==0:
            sphere_points=regionQuery(D,point,eps)
            numOfPoints=len(sphere_points)
            if numOfPoints>=MinPts:
                cluster+=1
                growCluster(D,my_labels,point,sphere_points,cluster,eps,MinPts)
            elif numOfPoints < MinPts:
                my_labels[point]=-1
    return my_labels
...
def growCluster(D, labels, P, NeighborPts, C, eps, MinPts):
    ...
    labels[P]=C
    P=0
    while P < len(NeighborPts):
        if labels[NeighborPts[P]] == 0:
            labels[NeighborPts[P]]=C
            sphere_points=regionQuery(D,NeighborPts[P],eps)
            numOfPoints = len(sphere_points)
            if numOfPoints>=MinPts:
                NeighborPts=NeighborPts+sphere_points
        elif labels[NeighborPts[P]] == -1:
            labels[NeighborPts[P]] = C
        P+=1
...
def regionQuery(D, P, eps):
    ...
    size=len(D)
    for point in range(size):
        dist= np.linalg.norm(D[point]-D[P])
        if dist<eps:
